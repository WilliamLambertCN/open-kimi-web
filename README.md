...
- **供应商模型能力**：可编辑供应商的每个模型都能用 checkbox 配置图片/视频、工具调用、思考与始终思考能力，以及 `none` 到 `max` 的全部思考档位。新模型或缺失配置默认全选，已有显式配置（包括空数组）保持原样；还可用当前表单的 Base URL 与可选 API Key 拉取 `/models`，从下拉框选择后加入模型列表。
