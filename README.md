...
- **会话与工作区整理**：工作区可从更多菜单置顶或取消置顶；在**设置 → 已归档会话**中，打开每条会话右侧的 `⋯` 菜单即可选择**永久删除**。置顶偏好只在当前浏览器保存工作区 ID；永久删除会先显示包含会话标题的不可撤销确认，并且仅在托管启动的官方后端上可用。更新 launcher 后要重启旧的 `kimi web` 进程，已经打开的旧服务不会自动加载这些功能。
