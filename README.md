...
- 供应商模型支持鼠标和触摸拖拽排序，保存后顺序与每行能力配置保持一致。
- 工作区支持置顶与取消置顶，多个置顶项按操作顺序排列；浏览器只保存工作区 ID。
- 已归档会话支持二次确认后永久删除；托管模式调用官方会话管理器完成数据清理。

### [open-kimi-web v0.41.0-r2](https://github.com/WilliamLambertCN/open-kimi-web/releases/tag/v0.41.0-r2) — 2026-09-08

- 首次访问默认使用夜幕主题，桌面主题入口移到左下角账号菜单的“会员升级”下方、“外观”上方；已有选择保持不变。
- 修复桌面账号菜单中的主题弹层被裁切、窄视口越界的问题，并确保只有当前主题显示勾选标记。
- 供应商模型新增能力与思考档位 checkbox；新增模型默认全选，已有显式配置原样保留。
- 供应商表单可从当前 Base URL 一键拉取 `/models`，通过下拉框选择并添加模型。
...
- **供应商模型能力**：可编辑供应商的每个模型都能用 checkbox 配置图片/视频、工具调用、思考与始终思考能力，以及 `none` 到 `max` 的全部思考档位。新模型或缺失配置默认全选，已有显式配置（包括空数组）保持原样；还可用当前表单的 Base URL 与可选 API Key 拉取 `/models`，从下拉框选择后加入模型列表，并用鼠标或触摸拖拽模型排序。
- **会话与工作区整理**：工作区可从更多菜单置顶或取消置顶；已归档会话可在明确确认后永久删除。置顶偏好只在当前浏览器保存工作区 ID；永久删除仅在托管启动的官方后端上可用。
- **五套氛围主题**：默认使用夜幕 · Nocturne；极光、暮色、余烬、矿物青绿也可在桌面账号菜单或手机设置中切换，覆盖页面、侧栏、输入区与弹层；也可随时恢复原始外观。
...
初次使用默认夜幕。主题选择（包括原始外观）保存在当前浏览器、当前站点的 `localStorage` 中，刷新后保持；已有选择不会被默认值覆盖，不同设备需要各自选择。选择原始外观会移除主题覆盖，恢复官方浅色、深色或跟随系统设置。浏览器禁止本地存储时仍可切换，但无法保证刷新后保留。
